...
typealias SelectionClosure = (Index, CTProfile) -> Void
typealias ConfigurationClosure = (Index, CTProfile) -> CTProfile
typealias CellConfigurationClosure = (Index, CTProfile, DropDownCell) -> Void
...
public final class DropDown: UIView {
  ...
  /**
	...
	*/
  var localizationKeysDataSource = [CTProfile]() {
    didSet {
  ...
  /**
	...
	*/
  convenience init(anchorView: AnchorView, selectionAction: SelectionClosure? = nil, dataSource: [CTProfile] = [], topOffset: CGPoint? = nil, bottomOffset: CGPoint? = nil, cellConfiguration: ConfigurationClosure? = nil, cancelAction: Closure? = nil) {
    self.init(frame: .zero)
...
extension DropDown {
  ...
  func selectRowAtIndex(index: Index?) {
    if let index = index {
  ...
  func deselectRowAtIndexPath(index: Index?) {
...
extension DropDown: UITableViewDataSource, UITableViewDelegate {
  ...
  private func configureCell(cell: DropDownCell, at index: Int) {
    if index >= 0 && index < localizationKeysDataSource.count {
      cell.accessibilityIdentifier = localizationKeysDataSource[index].firstName
    }
